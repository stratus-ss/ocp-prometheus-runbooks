...
Component(s): ETCD, OpenShift API, Kubernetes API
...
ETCD has not had a leader for more than 1 minute. ETCD is the database or “brains” that understands the state of the system. Without a leader, the cluster is placed into a read only state whereby no platform changes can be made. Existing workloads can continue to function (provided they do not crash), but no new actions are able to be taken. This can be caused by slow disk, communication problems, hardware failures or any number of other issues that prevent communication between ETCD members.

## Severity & Impact

This is a critical  situation and needs to be addressed immediately. The cluster is broken and will not serve its' intended function until this issue is resolved.



## SLO/SLI

There is no SLI, every event of this type is critical

## Diagnosis

### Logging

Logs are available in the cluster itself. However, the log forwarder has been configured to send non-application logs to Splunk.

If the cluster is in a read-only state you might need to SSH onto a host and use journalctl in order to retrieve the logs.

### Metrics/Dashboard Link

The default OpenShift dashboard can be found here. There is also a an external Dynatrace dashboard accessible to a larger audience.
...
* Network Issue
* Firewall/port blockage
* Slow disks on the ETCD Hosts
* VM or Physical host failures


## Hosts

With the cluster in a read only state it may still be possible to get information from the API.
